...
__device__ void PlaneHypothesisRefinement(const cudaTextureObject_t *images, const Camera *cameras, float4 *plane_hypothesis, float *depth, float *cost, curandState *rand_state, const float *view_weights, const float weight_norm, float *restricted_cost, const int2 p, const PatchMatchParams params)
{
    ...
    float4 plane_hypothesis_rand;
        ...
        plane_hypothesis_rand = GenerateRandomNormal(cameras[0], p, rand_state);
...
__device__ void CheckerboardPropagation(const cudaTextureObject_t *images, const cudaTextureObject_t *depths, const Camera *cameras, float4 *plane_hypotheses, float *costs, float *pre_costs, curandState *rand_states, unsigned int *selected_views, const int2 p, const PatchMatchParams params, const int iter)
{
    ...
    if (p.y > 2) {
        ...
        up_far = costMinPoint;
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_far], cost_array[1], params);//计算匹配代价并选择视图
    }
    ...
    PlaneHypothesisRefinement(images, cameras, &plane_hypotheses_now, &depth_now, &cost_now, &rand_states[center], view_weights, weight_norm, &restricted_cost, p, params);
    costs[center] = cost_now;
