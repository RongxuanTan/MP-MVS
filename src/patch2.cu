...
__global__ void InitializeScore(const cudaTextureObject_t* images, Camera* cameras, float4* PlaneHypotheses, float* costs, curandState* randStates, unsigned int *selected_views, const PatchMatchParams params){
    ...
    curand_init(clock64(), p.y, p.x, &randStates[center]);

    //随即生成平面假设
    if(!params.geom_consistency){
        PlaneHypotheses[center]=GenerateRandomPlaneHypothesis(cameras[0], p, &randStates[center], params.depth_min, params.depth_max);
        costs[center] = ComputeMultiViewInitialCostandSelectedViews(images, cameras, p, PlaneHypotheses[center], &selected_views[center], params);
    }else{
        float4 plane_hypothesis=PlaneHypotheses[center];
        ...
        PlaneHypotheses[center] = plane_hypothesis;
        costs[center] = ComputeMultiViewInitialCostandSelectedViews(images, cameras, p, PlaneHypotheses[center], &selected_views[center], params);
    }
...
__global__ void BlackPixelUpdate(const cudaTextureObject_t* images, const cudaTextureObject_t *depths, Camera *cameras, float4 *PlaneHypotheses, float *costs, float *pre_costs, curandState *randStates, unsigned int *selected_views, const PatchMatchParams params, const int iter){
    ...
    CheckerboardPropagation(images, depths, cameras, PlaneHypotheses, costs, randStates, selected_views, p, params, iter);
