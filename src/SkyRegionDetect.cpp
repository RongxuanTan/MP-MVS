...
SkySegment::SkySegment(const char* params_path, const char* model_path){
    skynet.load_param(params_path);
    skynet.load_model(model_path);
    skynet.opt.use_vulkan_compute = true;
}

cv::Mat SkySegment::maskExtractor(cv::Mat &dst){
